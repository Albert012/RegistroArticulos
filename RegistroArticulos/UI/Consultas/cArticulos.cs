...
namespace RegistroArticulos.UI.Consultas
{
    public partial class cArticulos : Form
    {
        ...
        /*private bool ValidarFiltro()//no cumplio con lo que realmente buscaba
        {
            bool paso = true;
            if (Filtro_comboBox.Text == string.Empty)
            {
                MessageBox.Show("Favor Seleccione Una Opcion De Consulta!!", "Error Campos Vacios!!", MessageBoxButtons.OK, MessageBoxIcon.Question);
                paso = false;
            }
            else
            if (Criterio_textBox.Enabled = true && Criterio_textBox.Text == string.Empty)
            {
                MessageBox.Show("Digite Un Criterio De Dicha Seleccion!!", "Error Campos Vacios!!", MessageBoxButtons.OK, MessageBoxIcon.Question);
                paso = false;
            }
            else
                paso = true;
